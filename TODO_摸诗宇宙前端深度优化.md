...
- **动态延迟策略**:
  - 第1列：固定250ms
  - 第2列：有匹配→400ms，无→250ms
  - 第3列：有匹配→500ms，无→250ms
  - 第4列：有匹配→600ms+轻微抖动，无→250ms
  - 第5列：有匹配→700ms+剧烈抖动，无→250ms
- **分层特效设计**:
  | 连线状态 | 触发条件 | 视觉特效 |
  |----------|----------|----------|
  | 无连线 | 该列断开 | 无光带，快速停止 |
  | 2连 | 第1-2列匹配 | 基础光带闪烁 |
  | 3连 | 第1-3列匹配 | 光带+边框呼吸增强 |
  | 4连 | 第1-4列匹配 | 光带+呼吸星星环绕 |
  | 5连 | 全部匹配 | 光带+星星+闪电扫过 |
- 交付物：
  - SlotMachine.vue - 动态延迟逻辑+连线检测
  - 新增特效组件或CSS（星星/闪电）
- 验收标准：
  - 连上的列延迟递增
  - 不同连线等级有明显视觉区分
...
- 完成状态：⏳ 待开始
- 执行步骤：
  - [ ] 步骤A.5.1：实现连线检测函数 `calculateChainLevel(matrix, colIdx)`
  - [ ] 步骤A.5.2：实现动态延迟逻辑（根据连线等级调整setTimeout）
  - [ ] 步骤A.5.3：实现条件性光带特效（只有连上的列才触发）
  - [ ] 步骤A.5.4：实现4连星星特效
  - [ ] 步骤A.5.5：实现5连闪电特效
  - [ ] 步骤A.5.6：验证
