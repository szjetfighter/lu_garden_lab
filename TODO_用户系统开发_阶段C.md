...
- 验收标准：
  - [ ] metadata信息统一在一个卡片区域
  - [ ] 包含：原诗、章节、创作时间、您的感受
  - [ ] 视觉层次清晰：metadata区（浅色背景）+ 诗歌内容区（白色卡片）
  - [ ] key-value布局紧凑、易读
...
- 执行步骤：
  - [ ] 步骤C.8.1：分析当前PoemViewer的metadata展示结构
  - [ ] 步骤C.8.2：设计统一的metadata卡片布局
    - 浅色背景（如 `var(--bg-secondary)`）
    - key-value紧凑布局
    - 合并原诗信息和用户输入
  - [ ] 步骤C.8.3：调整样式实现
  - [ ] 步骤C.8.4：测试视觉效果（桌面端+移动端）
...
- 交付物：
  - 后端：`DELETE /api/my-works/:workId` 接口
  - 前端：删除按钮 + 确认对话框

- 验收标准：
  - [ ] 每个作品卡片/列表项有删除按钮（如垃圾桶图标）
  - [ ] 点击删除触发二次确认对话框
  - [ ] 确认后调用后端API删除
  - [ ] 删除成功后刷新列表
  - [ ] 只能删除自己的作品（后端权限验证）
...
- **风险评估**：中风险（涉及数据删除，需要权限验证和错误处理）

- 预期改动文件（预判）：
  - `lugarden_universal/application/src/routes/myWorks.js`（新增DELETE接口）
  - `lugarden_universal/application/src/services/myWorksService.js`（删除逻辑）
  - `lugarden_universal/frontend_vue/src/core/auth/services/authApi.ts`（deleteWork API）
  - `lugarden_universal/frontend_vue/src/core/auth/views/MyWorksView.vue`（删除按钮+对话框）
...
- 技术设计：
  - **后端**：
    - 接口：`DELETE /api/my-works/:workId`
    - 权限验证：JWT token验证 + 作品所有权验证
    - 删除方式：软删除（推荐，可恢复）或硬删除
  - **前端**：
    - 删除按钮：图标按钮（🗑️），悬停提示"删除"
    - 确认对话框：简洁清晰，"确定删除《诗歌标题》吗？此操作不可恢复"
    - 成功反馈：Toast提示"已删除"

- 执行步骤：
  - [ ] 步骤C.10.1：后端实现DELETE接口
    - 路由注册
    - 权限验证（token + 作品所有权）
    - 删除逻辑（软删除或硬删除）
  - [ ] 步骤C.10.2：前端实现deleteWork API
  - [ ] 步骤C.10.3：UI添加删除按钮
  - [ ] 步骤C.10.4：实现确认对话框
  - [ ] 步骤C.10.5：错误处理和成功反馈
  - [ ] 步骤C.10.6：测试删除功能
