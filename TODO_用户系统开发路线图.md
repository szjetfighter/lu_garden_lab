...
> **重要架构调整**：本决策在首次设计时未考虑周全，在实施前通过架构复盘发现了单一数据库方案的多重隐患。

#### 🔴 初始设计（已废弃）：User和UserGongBiWork在lugarden.db

**初始方案**：
```
lugarden.db（单一数据库）
├── 内容表（ZhouPoem, ZhouMapping, Theme, Emotion...）
└── 用户表（User, UserGongBiWork）
```

**优点**：
- ✅ 外键约束有效（UserGongBiWork可以引用ZhouPoem和ZhouMapping）
- ✅ 查询简单（可以JOIN）
- ✅ 单一Prisma Client，开发简单

**致命问题发现过程**：

1. **运维隔离缺失**
   ```
   场景：用户疯狂注册/共笔 → 单一db写入频繁
   → SQLite写锁 → 整个lugarden.db被锁
   → 诗歌展示也受影响 ❌
   
   场景：VPS突然断电 → lugarden.db损坏
   → 无法判断是User表坏了还是ZhouPoem表坏了
   → 恢复困难 ❌
   ```

2. **Git管理混乱**
   ```
   现状：lugarden.db被Git跟踪（为了开发环境同步）
   
   风险1：用户密码hash被提交到GitHub ❌
   风险2：开发环境测试用户被同步到VPS生产环境 ❌
   风险3：Git pull覆盖VPS真实用户数据 ❌
   ```
...
4. **备份策略冲突**
   ```
   lugarden.db的两种数据性质：
   - 静态内容（诗歌、主题）→ Git管理，跟随代码部署
   - 用户数据（注册、作品）→ 定时备份，环境独立
   
   混在一起 → 无法分别管理 ❌
   ```

#### ✅ 正确方案：分离数据库（lugarden.db + auth.db）

**决策**：User和UserGongBiWork独立到auth.db

**数据库分配**：
```
lugarden.db（静态内容，Git跟踪）:
  - 所有内容表（ZhouPoem, ZhouMapping, Theme, Emotion...）
  - 写入频率：极低（只有西尔手动更新内容时）
  - 备份策略：Git管理，从Git部署
...
SQLite不支持跨数据库外键，因此UserGongBiWork无法直接引用ZhouPoem和ZhouMapping。
...
model UserGongBiWork {
  ...
  sourcePoemChapter String    // 章节
  mappingChapter String       // 章节
  mappingCombination String   // 答题组合（如"AA"）
...
**为什么数据冗余可接受？**
1. **源数据是静态的**：ZhouPoem和ZhouMapping不会频繁修改
2. **历史快照价值**：用户作品应该保留创作时的原诗状态（即使原诗后来被修改/删除）
3. **存储成本极低**：每条作品多存100字节，10万条=10MB
4. **运维价值 > 技术纯洁性**：单人运维，稳定性优先

**核心价值对比**：

| 价值维度 | 单一数据库 | 分离数据库 |
|---------|----------|-----------|
| **运维隔离** | ❌ 故障影响全局 | ✅ auth.db坏了不影响诗歌展示 |
| **备份策略** | ❌ 混在一起 | ✅ Git管理内容，定时备份用户数据 |
| **开源友好** | ❌ 需要清理用户数据 | ✅ lugarden.db开箱即用 |
| **数据安全** | ❌ 用户数据可能误提交 | ✅ auth.db永远不入Git |
| **环境隔离** | ❌ 开发/VPS共享用户数据 | ✅ 各环境独立用户库 |
| **故障排查** | ❌ 无法定位具体表 | ✅ 分别监控db大小 |

**部署策略**：
```bash
# 开发环境
prisma/schema.prisma       → lugarden.db（内容数据）
prisma/auth-schema.prisma  → auth.db（用户数据）

# Git跟踪
✅ schema.prisma, auth-schema.prisma  # 给开发者
✅ lugarden.db                         # 静态内容，所有人可用
❌ auth.db                             # 环境独立，不入Git

# VPS备份
lugarden.db → 从Git部署，无需备份
auth.db     → 每天3点备份到/backup/auth-YYYYMMDD.db
```

**技术权衡**：
- ✅ 优势：运维简单、数据安全、开源友好、故障隔离
- ⚠️ 代价：失去外键约束、数据轻微冗余、需要两个Prisma Client
- 🎯 结论：单人运维场景下，优势远大于代价
...
> **架构说明**：基于3.3节的分离决策，用户系统使用独立的auth.db
...
```
lugarden_universal/application/prisma/
├── schema.prisma       # 内容数据库schema（ZhouPoem, ZhouMapping...）
└── auth-schema.prisma  # 用户数据库schema（User, UserGongBiWork）
```
...
**字段说明**：
- **外部引用**：`sourcePoemId`、`mappingId`只存ID，无外键（跨数据库）
- **冗余字段**：从lugarden.db冗余关键信息，用于独立展示和历史快照
- **Dify追溯**：完整保存conversationId、messageId和usageMetadata，用于成本分析
