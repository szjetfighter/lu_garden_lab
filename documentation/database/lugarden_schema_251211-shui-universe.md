# 陆家花园主宇宙数据库设计 - v3.4 (水宇宙扩展)

> **基准版本**: 2025-12-11
> **文档状态**: ✅ 定稿
> **基于版本**: v3.3 (lugarden_schema_251127-scene-character.md)
...
> **数据治理结果**（实际数据统计）：
> - ShuiCollection: 12条记录（12个故事集）
> - ShuiPoem: 109条记录（含NLP tokens字段）
> - 数据质量审核: 全部通过
...
- **水宇宙：2张表（ShuiCollection、ShuiPoem）**
...
### 水宇宙实体（来源：poeject_shui_universe/data/）

> **本次新增** (2025-12-11)

#### 17) ShuiCollection（故事集表）
**数据来源：** `collections.json`
- id (PK): 故事集唯一标识 (shui_collection_bohai, shui_collection_taiyuan, etc.)
- name: 故事集中文名 ("渤海故事集", "太原故事集", etc.)
- nameEn: 故事集英文标识 ("bohai", "taiyuan", etc.)
- dedication: 献词（仅南溪故事集有）
- dateStart: 创作起始日期，格式"YYYY.M"
- dateEnd: 创作结束日期，格式"YYYY.M"
- regions (json): 涉及地域数组
- poemCount: 诗歌数量
- description: 故事集描述
- universeId (FK → Universe.id)
...
#### 18) ShuiPoem（诗歌表）
**数据来源：** `poems.json`
- id (PK): 诗歌唯一标识 (shui_poem_bohai_01, shui_poem_taiyuan_05, etc.)
- collectionId (FK → ShuiCollection.id)
- title: 诗题（多为地名或意象）
- content: 诗歌全文
- date: 创作日期，格式"YYYY.M.D"
- location: 主要地点（可选）
- index: 在故事集中的序号（从1开始）
- tokens (json): NLP分词结果（平均71.8 tokens/首）
- universeId (FK → Universe.id)
...
model ShuiPoem {
  ...
  location     String?
  index        Int
  tokens       String?         // JSON array - NLP分词结果
  universeId   String
...
> 说明：v3.4版本新增水宇宙Schema（ShuiCollection、ShuiPoem），待执行Prisma迁移。
